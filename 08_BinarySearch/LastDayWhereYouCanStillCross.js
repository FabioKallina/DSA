...
var latestDayToCross = function(row, col, cells) {
    ...
    let isPossible = ( mid ) => {
        let seen = Array.from({ length: row }, () => Array(col).fill(false));
        let flooded = Array.from({ length: row }, () => Array(col).fill(false));

        for ( let i = 0; i < mid; i++ ) {
            let [rr, cc] = cells[i];
            flooded[rr - 1][cc - 1] = true; //converting to 0-base index
        }
        let q = [];
        for ( let j = 0; j < col; j++ ) {
            if ( !flooded[0][j] ) {
